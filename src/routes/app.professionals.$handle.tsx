...
export function ProfessionalDetail() {
  const { handle } = useParams({ from: "/app/professionals/$handle" });
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (savedCerts) {
        try { setCertifications(JSON.parse(savedCerts)); } catch {}
      }
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
          ...
          <div className="flex items-start sm:items-center gap-5">
            <div className="relative shrink-0">
              <div className="size-20 sm:size-24 rounded-2xl bg-gradient-to-br from-emerald-500/20 to-neutral-900 border-2 border-emerald-500/30 flex items-center justify-center font-mono font-bold text-2xl text-emerald-400 shadow-xl">
                {initials}
              </div>
          ...
          {/* Capability Index Score Box & Edit Identity Modal */}
